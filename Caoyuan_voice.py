...
# x_Noi_1D[Ind[p1+p2:p1+p2+p3]] = x_Noi_1D[Ind[p1+p2:p1+p2+p3]] + (np.random.rand(p3)*4) -2 # Add uniform noise
...
X_Noi = scio.loadmat('MagSTFT.mat')['MagSTFT'].astype(float)
I, J = X_Noi.shape
...
model.initialise()
MagLR = model.run(iterations=maxIter)

MagSP = X_Noi - MagLR

scio.savemat('MagLR.mat', {'MagLR': MagLR})
scio.savemat('MagSP.mat', {'MagSP': MagSP})
