...
class KMeans:
    def __init__(self, X, M, K, resolve_empty='singleton'):
        ...
        assert len(
            self.X.shape) == 2, "Input matrix X is not a two-dimensional array, but instead %s-dimensional." % len(
            self.X.shape)
        assert self.X.shape == self.M.shape, "Input matrix X is not of the same size as the indicator matrix M: %s and %s respectively." % (
        self.X.shape, self.M.shape)
        ...
        if self.no_points < self.K: print
        "Want %s clusters but only have %s datapoints!" % (self.K, self.no_points)
        if self.no_unique_points < self.K: print
        "Want %s clusters but only have %s unique datapoints!" % (self.K, self.no_unique_points)
        ...
        self.omega_columns = [[i for i in range(0, self.no_points) if self.M[i, j]] for j in
                              range(0, self.no_coordinates)]
        ...
        if len(columns_to_remove) > 0:
            print
            "WARNING: removed columns %s for K-means clustering as they have no observed datapoints." % columns_to_remove
            ...
            self.omega_rows = [[j for j in range(0, self.no_coordinates) if self.M[i, j]] for i in
                               range(0, self.no_points)]
            self.omega_columns = [[i for i in range(0, self.no_points) if self.M[i, j]] for j in
                                  range(0, self.no_coordinates)]
    ...
    def cluster(self):
        ...
        while change:
            ...
            if iteration >= max_iterations:
                print
                "WARNING: did not converge, stopped after %s iterations." % max_iterations
                break
    ...
    def closest_cluster(self, data_point, index_data_point, mask_d):
        ...
        for c, (centroid, mask_c) in enumerate(zip(self.centroids, self.mask_centroids)):
            ...
            if (closest_MSE is None) or (
                    MSE is not None and MSE < closest_MSE):  # either no closest centroid yet, or MSE is defined and less
                closest_MSE = MSE
                closest_index = c
    ...
    def compute_MSE(self, x1, x2, mask1, mask2):
        ...
        return None if len_overlap == 0 else numpy.multiply(mask1_mask2,
                                                            numpy.power(numpy.subtract(x1, x2), 2)).sum() / float(
            len_overlap)
    ...
    def create_matrix(self):
        ...
        print[sum(column)
        for column in self.clustering_results.T]
