...
class MatrixNestedCrossValidation:
    def __init__(self, method, R, M, K, P, parameter_search, train_config, predict_config, file_performance,
    ...
    def run(self, parallel=True):
        ...
        folds_training, folds_test = folds_method(I=self.I, J=self.J, no_folds=self.K, attempts=attempts_generate_M,
                                                  M=self.M)

        for i, (train, test) in enumerate(zip(folds_training, folds_test)):
            print
            "Fold %s of nested cross-validation." % (i + 1)
            ...
            try:
                ...
                print
                "Best parameters for fold %s were %s." % (i + 1, best_parameters)
            except KeyError:
                best_parameters = self.parameter_search[0]
                print
                "Found no performances, dataset too sparse? Use first values instead for fold %s, %s." % (
                i + 1, best_parameters)
            ...
            print
            "Finished fold %s, with performances %s." % (i + 1, performance_dict)
