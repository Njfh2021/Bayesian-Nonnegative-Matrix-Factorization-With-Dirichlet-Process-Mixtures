...
def try_generate_M(I, J, fraction, attempts, M=None):
    ...
    for i in range(attempts):
        ...
        if check_empty_rows_columns(M_train):
            return M_train, M_test
    assert False, "Failed to generate folds for training and test data, %s attempts, fraction %s." % (
    attempts, fraction)
...
def compute_folds(I, J, no_folds, M=None):
    ...
    random.shuffle(indices)
    split_places = [int(i * no_elements / no_folds) for i in
                    range(0, no_folds + 1)]  # find the indices where the next fold start
    split_indices = [indices[split_places[i]:split_places[i + 1]] for i in
                     range(0, no_folds)]  # split the indices list into the folds
