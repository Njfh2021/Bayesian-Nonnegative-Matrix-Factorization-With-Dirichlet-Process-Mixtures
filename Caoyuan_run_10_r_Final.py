...
for x in xrange(1, 11):
    ...
    # Total_error = []
    for r in [5, 10, 15]:
        X_Ori = scio.loadmat('Syntheticdata/X_Ori_{}.mat'.format(x))['X_Ori'].astype(float)
        X_Noi = scio.loadmat('Syntheticdata/X_Noi_Gaussian_{}.mat'.format(x))['X_Noi_Gaussian'].astype(float)
        I, J = X_Noi.shape
        # for r in [4]:
        # print("r is: {}".format(r))
        r_array.append(r)
...
scio.savemat('2020GaussianTotalError.mat', {'totalError': total_Error})
scio.savemat('2020MixturemeanError.mat', {'meanError': meanError})
