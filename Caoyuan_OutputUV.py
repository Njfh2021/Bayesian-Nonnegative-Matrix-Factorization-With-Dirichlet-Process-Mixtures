...
# error = []
total_Error = np.zeros([10, 5])
total_Cost = np.zeros([10, 5])
for x in xrange(1, 2):
    print("{} -th iteration:".format(x))

    X_Ori = scio.loadmat('CaoyuanData/X_Ori_{}.mat'.format(x))['X_Ori'].astype(float)
    X_Noi = scio.loadmat('CaoyuanData/X_Noi_Gaussian_{}.mat'.format(x))['X_Noi_Gaussian'].astype(float)
