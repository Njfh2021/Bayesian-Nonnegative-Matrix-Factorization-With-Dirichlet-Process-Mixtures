...
def create_Data(i, rank):
    #random.seed(i)

    #output_folder = os.path.dirname(__file__) + "/Syntheticdata/"
    ...
    scio.savemat('X_Ori_{}_R{}.mat'.format(i, rank), {'X_Ori': true_R})
    #scio.savemat('X_Noi_Gaussian_{}_R{}.mat'.format(i, rank), {'X_Noi_Gaussian': R})
    ...
    #scio.savemat('X_Noi_Sparse_{}_R{}.mat'.format(i, rank), {'X_Noi_Sparse': X_Noi_Sparse})
    ...
    x_Noi_Mixture_1D[Ind[0:p1]] = x_Noi_Mixture_1D[Ind[0:p1]] + np.random.randn(p1) * 0.25
    # print("All good till Here")
    p2 = int(math.floor(I * J * 0.2))
    x_Noi_Mixture_1D[Ind[p1:p1 + p2]] = x_Noi_Mixture_1D[Ind[p1:p1 + p2]] + (
                np.random.rand(p2) * 10) - 5  # Add uniform noise
    p3 = int(math.floor(I * J * 0.2))
    x_Noi_Mixture_1D[Ind[p1 + p2:p1 + p2 + p3]] = x_Noi_Mixture_1D[Ind[p1 + p2:p1 + p2 + p3]] +( np.random.rand(
        p3) *4)-2 

    # (np.random.rand(p3)*4) -2 # Add uniform noise
    X_Noi_Mixture = x_Noi_Mixture_1D.reshape([I, J])
    scio.savemat('X_Noi_Mixture_{}_R{}.mat'.format(i, rank), {'X_Noi_Mixture': X_Noi_Mixture})
    R1 = X_Noi_Mixture
